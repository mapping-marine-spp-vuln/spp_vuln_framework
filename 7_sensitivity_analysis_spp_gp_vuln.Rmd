...
write_csv(spp_vuln_conv_rescale, here('int/spp_gp_vuln_convex_scoring.csv'))
```
...
write_csv(spp_vuln_conc_rescale, here('int/spp_gp_vuln_concave_scoring.csv'))
```
...
```{r}
df_long <- df %>%
  select(taxon, stressor, species, linear = vuln, convex = v_conv, concave = v_conc) %>%
  gather(shape, vuln, -taxon, -stressor, -species) %>%
  mutate(shape = factor(shape, levels = c('linear', 'convex', 'concave')))

ggplot(df_long, aes(x = stressor, y = vuln)) +
  theme_minimal() +
...
Here, arrange low to high, so top rank is a low vulnerability.  To avoid issues of identical scores getting different ranks, let's use a quantile instead of a rank, so lowest score(s) get 1, highest score(s) get 1000.
``` {r}
rmse_by_stressor <- df %>%
...
`r DT::datatable(rmse_by_stressor)`

### RMSE of rank by stressor and taxon

`r DT::datatable(rmse_by_taxon)`