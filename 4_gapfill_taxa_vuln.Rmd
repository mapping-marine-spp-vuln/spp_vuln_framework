...
if(!file.exists(full_class_file <- here('_data/spp_complete_classifications_worms.csv'))) {
...
spp_df_wide <- p_from_k %>%
  select(kingdom = parent) %>%
  distinct() %>%
  left_join(p_from_k %>% select(parent, phylum = name), 
            by = c('kingdom' = 'parent')) %>%
  left_join(c_from_p %>% select(parent, class = name), 
            by = c('phylum' = 'parent')) %>%
  left_join(o_from_c %>% select(parent, order = name), 
            by = c('class' = 'parent')) %>%
  left_join(f_from_o %>% select(parent, family = name), 
            by = c('order' = 'parent')) %>%
  left_join(g_from_f %>% select(parent, genus = name),
            by = c('family' = 'parent')) %>%
  left_join(s_from_g %>% select(parent, species = name), 
            by = c('genus' = 'parent')) %>%
  distinct()
...
``` {r}
knitr::kable(show_dupes(spp_df_wide %>% 
