...
Here we will pull in depth ranges from AquaMaps and include where possible.
...
z_lvls <- levels(depth_class_df$zone)
...
spp_depth_df <- spp_depth_df %>%
  mutate(z = as.integer(zone)) %>%
  group_by(species) %>%
  mutate(r = list(min(z):max(z))) %>%
  unnest(r) %>%
  mutate(zone = factor(z_lvls[r], levels = z_lvls)) %>%
  select(species, trait_value = zone) %>%
  unique() %>%
