...
get_downstream <- function(gp_df, db = 'worms', 
  ...
  tmp_files <- list.files(here('tmp'),
                          pattern = sprintf('%s_%s_.+.csv', namestem, db),
                          full.names = TRUE)
  
  tx_out_all <- parallel::mclapply(tmp_files, 
                                   FUN = function(f) {
                                     asdf <- read_csv(f) %>%
...
* _Lottia scutum_ and _Cellana tramoserica_ have no "order" rank, so they are dropping out.  How to reconcile that?!
* _Semirossia_ - ??? not sure why this is failing.
