...
#     filter(!is.na(rank) & rank %in% c('kingdom', 'phylum', 'class', 'order', 'family')) %>%
#     .$name %>% 
#     unique() %>%
#     tolower()
#   return(ffffffuuuuuu)
...
check_status <- function(ids) {
  n_chunks <- ceiling(length(ids) / 50)
  records_list <- vector('list', length = n_chunks)
  for(i in 1:n_chunks) {
    indices <- ((i-1)*50 + 1):min(i*50, length(ids))
    ids_chunk <- ids[indices]
