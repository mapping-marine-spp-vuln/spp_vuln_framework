...
## What if multiple random spp

What if x vec of deterministic vulns and y vec of random vulns with length > 1?

Use three parallel vecs v, sd, and n.  Collapse non-NA/non-zero sd values into a single dist as y.
...
x <- rnorm(10, mu_x, sd_x)
y <- rnorm(5, mu_y, sd_y)

v <- c(x, y)
sd_v <- c(rep(NA, 10), runif(5, .5, 1.5))
n_v <- rpois(15, 1) %>% ifelse(. == 0, 1, .)

calc_var2 <- function(v, sd_v, n_v) {
  df <- data.frame(v, sd_v, n_v)
  x <- df %>% 
    filter(sd_v == 0 | is.na(sd_v)) %>%
    uncount(n_v) %>%
    .$v
  y_df <- df %>% 
    filter(sd_v > 0 & !is.na(sd_v)) %>%
    uncount(n_v)
  y <- y_df$v; sd_y <- y_df$sd_v
  
  asdf <- vector('numeric', length = 1000)
  for(i in 1:1000) {
    y_samp <- y_df %>%
      rowwise() %>%
      mutate(y = rnorm(1, v, sd_v))
    asdf[i] <- var(y_samp$y)
  }
  y_mean <- mean(y_df$v)
  y_var <- sqrt(sum(y_df$sd_v^2))
