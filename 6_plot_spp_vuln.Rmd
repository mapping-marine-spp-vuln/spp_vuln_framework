...
```{r taxa_12x1panel_boxplot_w_jitter, eval = FALSE}
...
x <- ggplot(plot_df %>% mutate(str_clean = fct_rev(str_clean)), 
  ...
  scale_y_continuous(breaks = c(0.07, 0.93), labels = c('0', '1')) +
  ### boxplot
  geom_boxplot(color = 'grey40', size = .2, fill = NA, outlier.size = .05, outlier.color = 'grey70') +
  geom_jitter(color = 'blue', size = .1, alpha = .1) +
  geom_hline(data = tx_summary_df, 
             aes(yintercept = v_mean_all), color = 'red', size = .1) +
  geom_point(data = tx_summary_df, aes(y = v_mean), 
             shape = 21, color = 'black', fill = 'red', size = 1) +
