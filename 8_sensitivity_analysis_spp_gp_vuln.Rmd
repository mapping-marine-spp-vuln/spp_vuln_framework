...
df <- df_raw %>%
  ...
  left_join(str_fixes, by = c('stressor')) %>%
...
```{r}
df_long <- df %>%
  select(tx_lbl, str_lbl, species, 
         linear = vuln, convex = v_conv, concave = v_conc) %>%
  gather(shape, vuln, -tx_lbl, -str_lbl, -species) %>%
  mutate(shape = factor(shape, levels = c('convex', 'linear', 'concave')))

p <- ggplot(df_long, aes(x = str_lbl, y = vuln)) +
  theme_ohara() +
  theme(axis.title.y = element_blank()) +
  geom_boxplot(aes(fill = shape), size = .1,
               outlier.size = .1, outlier.color = 'grey70') +
  scale_fill_viridis_d() +
  coord_flip() +
  labs(#title = 'Vulnerability by stressor and score function',
       fill = 'function', y = 'Vulnerability')

ggsave('figs/si_sensitivity_str.png', height = 3.5, width = 6, dpi = 300)
knitr::include_graphics('figs/si_sensitivity_str.png')

p <- ggplot(df_long, aes(x = tx_lbl, y = vuln)) +
  theme_ohara() +
  theme(axis.title.y = element_blank()) +
  geom_boxplot(aes(fill = shape), size = .1,
               outlier.size = .1, outlier.color = 'grey70') +
...
ggsave('figs/si_sensitivity_tx.png', height = 2.5, width = 6, dpi = 300)
knitr::include_graphics('figs/si_sensitivity_tx.png')
...
p <- ggplot(plot_str_df, aes(x = str_lbl, y = rmse_rank, color = score_f)) +
  geom_point() +
  theme_ohara() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        axis.title.x = element_blank()) +
  scale_color_manual(values = c("#4B0055", "#00BE7D")) +
  labs(y = 'RMSE(rank)',
       color = 'score function')
ggsave('figs/si_sensitivity_rank_str.png', height = 2.5, width = 6, dpi = 300)
knitr::include_graphics('figs/si_sensitivity_rank_str.png')
```
...
plot_str_tx_df <- rmse_rank_by_taxon %>%
  select(tx_lbl, starts_with('rmse_rank')) %>%
  gather(score_f, rmse_rank, -tx_lbl) %>%
  mutate(score_f = ifelse(score_f == 'rmse_rank_conc', 'concave', 'convex')) %>%
  group_by(tx_lbl) %>%
  mutate(mean_rmse_rank = mean(rmse_rank)) %>%
  arrange(mean_rmse_rank) %>%
  ungroup() %>%
  mutate(tx_lbl = fct_inorder(tx_lbl))
  
ggplot(plot_str_tx_df, aes(x = tx_lbl, y = rmse_rank, color = score_f)) +
  geom_point() +
  theme_ohara() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        axis.title.x = element_blank()) +
  ...
  labs(y = 'RMSE(rank)',
       color = 'score function',
       title = 'RMSE(rank) by taxon')

ggsave('figs/si_sensitivity_rank_tx.png', height = 2.5, width = 6, dpi = 300)
knitr::include_graphics('figs/si_sensitivity_rank_tx.png')
