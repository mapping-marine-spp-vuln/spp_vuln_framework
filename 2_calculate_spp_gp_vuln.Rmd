...
sd_fun <- function(s, p) {
  if(all(p == 1)) return(NA)
  
  s_bar <- sum(s * p, na.rm = TRUE)
  s_sd <- sqrt(sum((s - s_bar)^2) / (length(s) - 1))
  return(s_sd)
}
...
for(spp in spp_mc) {
  ...
  # iters <- max(1000, n_combos * 2)
  iters <- 1000
  sample_list <- vector('list', length = iters)
  
  for(i in 1:iters) {
    trait_sample <- trait_multi %>%
      group_by(trait) %>%
      slice_sample(n = 1) %>%
    ...
    spp_sens_sample <- trait_sample %>%
      left_join(spp_sens_raw %>% select(-trait_prob), 
                by = c('taxon', 'category', 'trait', 'trait_value', 'spp_gp')) %>%
      group_by(spp_gp, stressor, taxon) %>%
      summarize(sens_score = sum(sens_score)) %>%
      filter(!is.na(stressor))
    spp_adcap_gen_sample <- trait_sample %>%
      left_join(spp_adcap_gen_raw %>% select(-trait_prob), 
                by = c('taxon', 'category', 'trait', 'trait_value', 'spp_gp')) %>%
      group_by(spp_gp, taxon) %>%
      summarize(adcap_gen_score = sum(adcap_gen_score))
    spp_adcap_spec_sample <- trait_sample %>%
      left_join(spp_adcap_spec_raw %>% select(-trait_prob),
                by = c('taxon', 'category', 'trait', 'trait_value', 'spp_gp')) %>%
      group_by(spp_gp, stressor, taxon) %>%
      summarize(adcap_spec_score = sum(adcap_spec_score)) %>%
      filter(!is.na(stressor))

    spp_vuln_sample <- spp_sens_sample %>%
      left_join(spp_adcap_gen_sample, by = c('taxon', 'spp_gp')) %>%
      left_join(spp_adcap_spec_sample, by = c('taxon', 'spp_gp', 'stressor'))
    
    sample_list[[i]] <- spp_vuln_sample
  }
  
### NEED TO SAVE MEAN/SD SENS SCORE, ADCAP GEN, ADCAP SPEC THEN CALC FROM THOSE?!
  x <- bind_rows(sample_list)
    
    
    group_by(taxon, spp_gp, stressor) %>%
    summarize(max_sens = max(sens_score), 
                mean_sens = mean(sens_score),
                sd_sens = sd(sens_score),
              max_adcap_gen = max(adcap_gen_score),
                mean_acg = mean(adcap_gen_score),
                sd_acg = sd(adcap_gen_score),
              max_adcap_spec = max(adcap_spec_score),
                mean_acs = mean(adcap_spec_score),
...
%>%
  left_join(spp_dep_habs, by = c('taxon', 'spp_gp', 'stressor')) %>%
  ### fix NAs
...
%>%
  ...
  mutate(max_sens = ifelse(max_sens < 1, 1, max_sens)) %>%
    ### don't normalize if max observed value is already low (less than 1)
  ungroup() %>%
