...
spp_mc_vuln_list <- parallel::mclapply(seq_along(spp_mc), mc.cores = 30, 
  FUN = function(i) {
    ...
    sens_score_det <- sens_clean_det %>%
      group_by(stressor) %>%
      summarize(sens_score_det = sum(sens_score))
    
    if(nrow(sens_clean_multi) > 0) {
      spp_sens_sample <- sens_clean_multi %>%
        group_by(trait, stressor) %>%
        mutate(score_samp = list(sample(sens_score, size = iters, replace = TRUE))) %>%
        select(-trait_value, -sens_score) %>%
        distinct() %>%
        unnest(score_samp) %>%
        mutate(n = 1:n()) %>%
        group_by(stressor, n) %>%
        summarize(sens_score_random = sum(score_samp)) %>%
    ...
    acs_score_det <- acs_clean_det %>%
      group_by(stressor) %>%
      summarize(acs_score_det = sum(adcap_spec_score))
    
    if(nrow(acs_clean_multi) > 0) {
      spp_acs_sample <- acs_clean_multi %>%
        group_by(trait, stressor) %>%
        mutate(score_samp = list(sample(adcap_spec_score, size = iters, replace = TRUE))) %>%
        select(-trait_value, -adcap_spec_score) %>%
        distinct() %>%
        unnest(score_samp) %>%
        mutate(n = 1:n()) %>%
        group_by(stressor, n) %>%
        summarize(acs_score_random = sum(score_samp)) %>%
    ...
    if(nrow(acg_clean_multi) > 0) {
      spp_acg_sample <- acg_clean_multi %>%
        group_by(trait) %>%
        mutate(score_samp = list(sample(adcap_gen_score, size = iters, replace = TRUE))) %>%
        select(-trait_value, -adcap_gen_score) %>%
        distinct() %>%
        unnest(score_samp) %>%
        mutate(n = 1:n()) %>%
        group_by(n) %>%
        summarize(acg_score_random = sum(score_samp)) %>%
