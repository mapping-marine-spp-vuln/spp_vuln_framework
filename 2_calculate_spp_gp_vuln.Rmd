...
for(i in seq_along(spp_mc)) {
  ...
  sens_raw_tmp <- spp_sens_raw %>% 
    filter(spp_gp == spp) %>%
    select(trait, trait_value, sens_score, stressor)
  acg_raw_tmp <- spp_adcap_gen_raw %>% 
    filter(spp_gp == spp) %>%
    select(trait, trait_value, adcap_gen_score)
  acs_raw_tmp <- spp_adcap_spec_raw %>% 
    filter(spp_gp == spp) %>%
