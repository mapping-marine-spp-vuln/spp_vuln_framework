...
Load the vulnerability data, and match to the wide-format species classifications along with the representativeness corrections, to identify species that represent higher ranks.  Create a separate dataframe of these higher ranks, based on averages of species vulnerability scores.  

Then bind the representative-based dataframe with the original vulnerability data, and join to long-format species classifications.

```{r expand Beth fish representativeness, eval = FALSE}
fish_spp <- spp_gp_vuln %>%
  filter(taxon == 'fish') %>%
  select(species = spp_gp) %>%
  distinct()

fish_gps <- readxl::read_excel(here('_raw_data/xlsx/fish_traits_add_bp.xlsx'), 
                               sheet = 'repres_raw') %>%
  .$species_group %>% tolower()
  
fish_repres <- spp_all_wide %>%
  filter(species %in% fish_spp$species) %>%
  mutate(repres = case_when(genus %in% fish_gps  ~ 'genus',
                            family %in% fish_gps ~ 'family',
                            order %in% fish_gps  ~ 'order',
                            class %in% fish_gps  ~ 'class',
                            TRUE ~ 'species')) %>%
  mutate(taxon = 'fish') %>%
  select(taxon, species, repres) %>%
  distinct()

write_csv(fish_repres, 'fish_repres.csv')
...
repres_fish <- readxl::read_excel(here('_raw_data/xlsx/fish_traits_add_bp.xlsx'), 
...
### group by the new spp gp and summarize vulnerabilities
repr_spp_gps <- repr_spp %>%
  group_by(taxon, spp_gp_new, stressor) %>%
  summarize(sd_vuln = sqrt(sd(vuln)^2 + 1/n()*sum(sd_vuln^2, na.rm = TRUE)),
            vuln = mean(vuln),
            n_spp = n()) %>%
  ungroup() %>%
...
``` {r match up}
### add representative spp gps to the original spp_gp_vuln. Note
### this will keep the species level scores intact, while also adding
### higher order representative ranks.
spp_gp_vuln_repr_fix <- spp_gp_vuln %>%
  bind_rows(repr_spp_gps %>% select(-n_spp)) %>%
  clean_scinames('spp_gp')

matched_df <- spp_all_long %>%
  inner_join(spp_gp_vuln_repr_fix, by = c('name' = 'spp_gp')) %>%
  mutate(match = rank)
...
unmatched_df <- spp_gp_vuln_repr_fix %>%
...
DT::datatable(unmatch_check_df)
```

Notes: 

* _Manta_ is accepted as a genus, but all spp within it are accepted as _Mobula_
* _Balaenoptera brydei_ is taxon inquirendum
* _Leptopentacta elongata_ is accepted as _Paraleptopentacta elongata_ (already included)
* _Nautilus repertus_ is taxon inquirendum
* _Sousa chinensis taiwanensis_ is a subspecies (species is separately included)
...
match_spp <- match_vuln_df %>%
  ...
  mutate(downfill_spp_id = 1:n())

spp_vuln <- match_vuln_df %>%
  select(species, taxon, stressor, vuln, sd_vuln) %>%
  distinct() %>%
  left_join(match_spp %>% select(taxon, species, downfill_spp_id), by = c('taxon', 'species')) %>%
  mutate(vuln = round(vuln, 5), sd_vuln = round(sd_vuln, 5)) %>%
  select(-taxon, -species) ### get these back by matching with downfill_spp_id
