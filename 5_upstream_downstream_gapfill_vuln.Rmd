...
if(!file.exists(gen_fill_file)) {
  ...
  genera_list <- parallel::mclapply(genera, mc.cores = 20,
...
if(!file.exists(fam_fill_file)) {
  ...
  fam_list <- parallel::mclapply(fams, mc.cores = 20,
    FUN = function(f) {### 
      f_df <- fam_fill_prob %>%
        filter(family == f) %>%
        mutate(sd_vuln = ifelse(is.na(sd_vuln), 0, sd_vuln))
...
if(!file.exists(ord_fill_file)) {
  ...
  ord_list <- parallel::mclapply(ords, mc.cores = 15,
    FUN = function(o) {
      o_df <- ord_fill_prob %>%
        filter(order == o) %>%
        mutate(sd_vuln = ifelse(is.na(sd_vuln), 0, sd_vuln))
...
if(!file.exists(cls_fill_file)) {
  ...
  cls_list <- parallel::mclapply(clss, mc.cores = 1, ### memory issues
    FUN = function(c) {
      c_df <- cls_fill_prob %>%
        filter(class == c) %>%
        mutate(sd_vuln = ifelse(is.na(sd_vuln), 0, sd_vuln))
      ...
      FUN = function(s) {
        ...
        mc_tx_str_df <- taxon_str_df %>%
          ...
          summarize(mean_vuln = mean(v), sd_vuln = sd(v),
                    mean_vuln_known  = sum(v * !is.na(match_rank)) / sum(!is.na(match_rank)),
