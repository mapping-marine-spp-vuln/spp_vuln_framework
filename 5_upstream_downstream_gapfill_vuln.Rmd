...
vuln_vals <- vuln_all_spp %>%
  ...
  select(tx_id, stressor, vuln, sd_vuln, med_vuln) %>%
...
```{r}
vuln_vals <- read_csv('~/git-annex/spp_vuln/vuln_gapfill_values.csv')
vuln_tx <- read_csv('~/git-annex/spp_vuln/vuln_gapfill_taxa.csv')
vuln_df <- vuln_tx %>%
  left_join(vuln_vals, by = 'tx_id') %>%
  select(-tx_id, -n_known_spp, -n_prob) %>%
  select(-med_vuln) %>%
  # filter(order == 'anguilliformes') %>% ### work with a subset for now
  distinct()
