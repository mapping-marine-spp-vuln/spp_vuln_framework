...
These have already been scrubbed of confounding symbols and parenthetical genera... to avoid problems of tiny differences in mean and sd, round to 4 decimal places.  There are also several conflicting taxonomic classifications of families with multiple orders (WoRMS order noted):

* ophiacanthidae (ophiacanthida)
* labridae (eupercaria incertae sedis)
* apogonidae (kurtiformes)

Coerce these appropriately - each spp is listed only for one order, so no additional wrangling is needed.

```{r load expanded vuln scores from script 4}
match_spp <- read_csv(here('int/spp_match_downfill.csv'), show_col_types = FALSE)
spp_vuln  <- read_csv(here('int/vuln_downfill_only.csv'), show_col_types = FALSE)
...
match_vuln_df <- match_spp %>%
  ...
  mutate(vuln    = round(vuln, 4),
         sd_vuln = round(sd_vuln, 4))
...
This code takes ranks where all species in the rank have a deterministic vulnerability score for a given stressor (i.e., no variation in imputed vulnerability due to trait variation), and then summarizes across these species to get a mean and standard deviation for that rank and that stressor.
...
If any species in the taxon has a distribution around its vulnerability for a stressor (i.e., sd(vulnerability) != 0, due to multiple trait values), check a few things:
...
# unlink(gen_fill_file)
if(!file.exists(gen_fill_file) | reload) {
  
  if(force) {
    unlink(list.files(here('tmp'), pattern = 'gen_fill_mc.+.csv', full.names = TRUE))
  }
  ...
  trash <- parallel::mclapply(genera, mc.cores = 12,
  ...
  genera_fs <- list.files(here('tmp'), pattern = 'gen_fill_mc.+.csv',
                            full.names = TRUE)
  genera_list <- parallel::mclapply(genera_fs, FUN = read_csv, show_col_types = FALSE, mc.cores = 20)
  gen_fill_prob_results <- genera_list %>%
    bind_rows() 
  
  if(nrow(gen_fill_prob_results) != nrow(gen_fill_qty)) {
    stop('Mismatch row counts in genus level MC fill!') 
  }
  gen_fill_prob_results <- gen_fill_prob_results %>%
...
```{r}
gen_fill_prob_results <- read_csv(gen_fill_file, show_col_types = FALSE)
...
check <- gen_fill_totes %>%
  group_by(genus) %>%
  summarize(n_str = n_distinct(stressor), n_vals = sum(!is.na(mean_vuln)))
```
...
### unlink(fam_fill_file)
if(!file.exists(fam_fill_file) | reload) {
  
  if(force) {
    unlink(list.files(here('tmp'), pattern = 'fam_fill_mc.+.csv', full.names = TRUE))
  }
  ...
  trash <- parallel::mclapply(fams, mc.cores = 8,
  ...
  family_fs <- list.files(here('tmp'), pattern = 'fam_fill_mc.+.csv',
                            full.names = TRUE)
  fam_list <- parallel::mclapply(family_fs, FUN = read_csv, show_col_types = FALSE, mc.cores = 20)  
  fam_fill_prob_results <- fam_list %>%
    bind_rows()

  if(nrow(fam_fill_prob_results) != nrow(fam_fill_qty)) {
    stop('Mismatch row counts in family level MC fill!') 
  }
  
  fam_fill_prob_results <- fam_fill_prob_results %>%
...
```{r}
fam_fill_prob_results <- read_csv(fam_fill_file, show_col_types = FALSE)
...
iters <- 5000 ### set to fewer iterations - should be plenty
ord_fill_file <- here('int', sprintf('gapfill_ord_prob_results_%s.csv', iters))
### unlink(ord_fill_file)

if(!file.exists(ord_fill_file) | reload) {
  
  if(force) {
    unlink(list.files(here('tmp'), pattern = 'ord_fill_mc.+.csv', full.names = TRUE))
  }
  ...
  # trash <- parallel::mclapply(ords, mc.cores = 1, ### memory intensive!
  trash <- lapply(ords,
  ...
  ord_fs <- list.files(here('tmp'), pattern = 'ord_fill_mc.+.csv',
                            full.names = TRUE)
  ord_list <- parallel::mclapply(ord_fs, FUN = read_csv, show_col_types = FALSE, mc.cores = 20)
  
  ord_fill_prob_results <- ord_list %>%
    bind_rows()

  if(nrow(ord_fill_prob_results) != nrow(ord_fill_qty)) {
    stop('Mismatch row counts in order level MC fill!') 
  }
  
  ord_fill_prob_results <- ord_fill_prob_results %>%
...
```{r combine order gapfills}
ord_fill_prob_results <- read_csv(ord_fill_file, show_col_types = FALSE)
...
### unlink(cls_fill_file)

if(!file.exists(cls_fill_file) | reload) {

  if(force) {
    unlink(list.files(here('tmp'), pattern = 'cls_fill_mc.+.csv', full.names = TRUE))
  }
  ...
  trash <- parallel::mclapply(clss, mc.cores = 1, ### memory intensive!
  ...
  cls_fs <- list.files(here('tmp'), pattern = 'cls_fill_mc.+.csv',
                            full.names = TRUE)
  cls_list <- parallel::mclapply(cls_fs, FUN = read_csv, show_col_types = FALSE, mc.cores = 20)  
  
  cls_fill_prob_results <- cls_list %>%
    bind_rows()

  if(nrow(cls_fill_prob_results) != nrow(cls_fill_qty)) {
    stop('Mismatch row counts in class level MC fill!') 
  }
  
  cls_fill_prob_results <- cls_fill_prob_results %>%
...
```{r}
cls_fill_prob_results <- read_csv(cls_fill_file, show_col_types = FALSE)
...
vuln_vals <- read_csv(file.path(dir_mazu, 'pre_gapfill/vuln_gapfill_values.csv'), show_col_types = FALSE)
vuln_tx   <- read_csv(file.path(dir_mazu, 'pre_gapfill/vuln_gapfill_taxa.csv'), show_col_types = FALSE)
vuln_df <- vuln_tx %>%
  oharac::dt_join(vuln_vals, by = 'tx_id', type = 'left') %>%
...
These are just those species gapfilled to the family level (though include many downfilled from order or class).  These are the values reported in Nat's paper.
...
tx_str_summary <- read_csv(tx_str_sum_outfile, show_col_types = FALSE)
