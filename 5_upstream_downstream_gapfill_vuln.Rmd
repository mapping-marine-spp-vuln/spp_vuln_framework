...
gen_fill_file <- here('int', sprintf('gen_fill_prob_results_%s.csv', iters))
...
if(!file.exists(gen_fill_file)) {
  ...
  for(g in genera) { ### g <- genera[1]
    ...
    tmp_list <- parallel::mclapply(1:iters, mc.cores = 30,
      FUN = function(i) { ### i <- 2
        samp_df <- g_df %>%
          rowwise() %>%
...
fam_fill_file <- here('int', sprintf('fam_fill_prob_results_%s.csv', iters))
...
ord_fill_file <- here('int', sprintf('ord_fill_prob_results_%s.csv', iters))
...
cls_fill_file <- here('int', sprintf('cls_fill_prob_results_%s.csv', iters))
...
write_csv(vuln_gf_tx, '~/git-annex/spp_vuln/post_gapfill/vuln_gapfilled_all_tx.csv')
write_csv(vuln_gf_score, '~/git-annex/spp_vuln/post_gapfill/vuln_gapfilled_all_score.csv')
write_csv(vuln_gf_sd, '~/git-annex/spp_vuln/post_gapfill/vuln_gapfilled_all_sd.csv')
...
if(!file.exists(tx_str_sum_outfile)) {
  ...
  for(t in tx) { ### t <- tx[1]
    ...
    for(s in str) { ### s <- str[1]
      tmp2_df <- tmp_df %>%
        filter(stressor == s)
      n_known <- sum(tmp2_df$gapfill == 'none')
      n_total <- n_distinct(tmp2_df$species)
      mc_tx_str_df <- mc_iters(tmp2_df, iters = iters, mc.cores = 40) %>%
        group_by(taxon, stressor) %>%
        summarize(mean_vuln = mean(v), sd_vuln = sd(v),
                  n_spp_known = n_known,
                  n_spp_total = n_total,
                  pct_spp_filled = round((n_total - n_known)/n_total, 5) * 100)
