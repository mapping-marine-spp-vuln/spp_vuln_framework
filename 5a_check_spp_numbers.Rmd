...
```{r}
spp_downfill <- read_csv(here('_output/vuln_gapfilled_tx.csv'))
...
* Crustacea/arthropods, because of order of magnitude differences
...
birds_from_xlsx <- readxl::read_excel(xlsx_f, sheet = 'seabirds', skip = 1) %>%
  ...
  # mutate(species = str_replace(species, '^ardenna', 'puffinus'),
  #        species = ifelse(str_detect(species, 'hydrobates') & !str_detect(species, 'furcatus|pelagicus'),
  #                         str_replace(species, 'hydrobates', 'oceanodroma'), species),
  #        species = str_replace(species, 'gulosus|leucocarbo|microcarbo|nannopterum|^urile', 'phalacrocorax'),
  #        family = ifelse(family == 'oceanitidae', 'hydrobatidae', family),
  #        family = ifelse(family == 'oceanitidae', 'hydrobatidae', family),
  #        family = ifelse(str_detect(species, 'phalaropus'), 'phalaropodidae', family)) %>%
...
Notes:  These fixes have been implemented in the XSLX spreadsheet and code above for improved matching.  The only fix necessary to capture missing species is Scolopacidae -> Phaloropodidae.

* _Ardenna_: Genus _Ardenna_ Reichenbach, 1853 accepted as _Puffinus_ Brisson, 1760
* _Pelecanoididae_ - 5 spp that on the xlsx are listed as Procellariidae
* _Oceanitidae_ accepted as _Hydrobatidae_ Mathews, 1913 - but still only two _Hydrobatidae_?
* Most _Hydrobates_ species (on xlsx) are listed as _Oceanodroma_ species (e.g., Band-rumped Storm-petrel)
* Phalaropes are listed as family _Phalaropodidae_, not _Scolopacidae_ (but fix needs to be implemented earlier - not captured in species traits)
* _Gulosus aristotelis_ recognized as _Phalacrocorax aristotelis_; most _leucocarbo_, _microcarbo_, _urile_, and _nannopterum_ species are recognized as _phalacrocorax_ as well...

```{r}
worms <- assemble_worms()
```
