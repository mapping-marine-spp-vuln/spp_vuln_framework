...
get_incl_spp <- function(api_V = api_version, include_all_risk = FALSE) {
  ...
  sens_file <- here('_data/from_bd_chi', sprintf('spp_sensitivity_%s.csv', api_V))
  risk_file <- here('_data/from_bd_chi', sprintf('iucn_risk_current_%s.csv', api_V))
  maps_file <- here('_data/from_bd_chi', sprintf('spp_marine_maps_%s.csv', api_V))
  comp_file <- here('_data/from_bd_chi', sprintf('iucn_comp_assessed_%s.csv', api_V))
...
get_spp_taxon <- function(id = NULL, all_cols = FALSE) {
  taxa_descs <- read_csv(here('_data/from_bd_chi/taxa_descriptions.csv'),
                         col_types = cols(.default = 'c'))

  taxon_f <- here('_data/from_bd_chi/spp_ranks.csv')
